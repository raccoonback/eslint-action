...
const { GITHUB_SHA, GITHUB_EVENT_PATH, GITHUB_TOKEN, GITHUB_WORKSPACE, BASE_DIR, MATCHER } = process.env
...
function eslint() {
  ...
  const report = cli.executeOnFiles([GITHUB_WORKSPACE + '/' + BASE_DIR +'/' + MATCHER])
  ...
  for (const result of results) {
    ...
    const path = filePath.substring((GITHUB_WORKSPACE + '/' + BASE_DIR).length + 1)
